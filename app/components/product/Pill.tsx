...
export default function ProductPill({
  ...
  return (
    <Link onClick={onClick} to={`/products/${storefrontProduct.handle}`}>
      <div
        className={clsx(
          '-md group flex h-[108px] gap-4 border border-lightGray bg-white p-3 text-sm duration-500 ease-out',
          'hover:-lg hover:border-darkGray',
        )}
        role="row"
      >
        <div role="cell" className="relative flex-shrink-0">
          <div
            className={clsx(
              '-sm relative h-full w-[110px] overflow-hidden bg-lightGray duration-500 ease-out',
              'group-hover:-md',
            )}
          >
            {image && (
  ...
  return (
    ...
    >
      <div role="cell" className="relative flex-shrink-0">
        <div
          className={clsx(
            '-sm relative h-full w-[110px] overflow-hidden bg-gray duration-500 ease-out',
            'group-hover:-md',
          )}
        ></div>
      </div>
      ...
      <div className="overflow-hidden">
        <div className="mr-3 space-y-1">
          ...
          {/* Product options */}
          <div className="-full mb-2 h-3 w-30 bg-gray"></div>
          <div className="-full mb-2 h-3 w-18 bg-gray"></div>
        </div>

        {/* Price */}
        <div className="mt-4 flex">
          <div className="-full h-3 w-24 bg-gray"></div>
        </div>
      </div>
    </Skeleton>
  );
